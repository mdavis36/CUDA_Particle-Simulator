...
Particle::Particle(glm::vec3 p)//Integrator *i)
{
	//integrator = i;
	if (!init(p)) {
		cout << "Failed to initialize Particle" << endl;
...
void Particle::update(float time_step, glm::vec3 global_forces, Integrator *i)
{
...
void Particle::draw()
{
	//obviously this is where we will draw the particle using Open GL
